...
import io.aklivity.zilla.specs.engine.internal.types.stream.*;
...
public class TestWelcomePathBindingFactory implements BindingHandler {
    ...
    private final Logger logger = LoggerFactory.getLogger(TestWelcomePathBindingFactory.class);

    TestWelcomePathBindingFactory(
                EngineContext context)
        {
            this.context = context;
            this.router = new Long2LongHashMap(-1L);
            logger.debug("TestWelcomePathBindingFactory instance created. @params EngineContext");
        }
                ...
                BindingConfig binding)
        {
            ...
            if (exit != null)
            {
                ...
                logger.debug("exit mapped: " + binding.id + " " + exit.id);
            }
        }
        ...
        public void detach(
                BindingConfig binding)
        {
            logger.debug("removing binding: " + binding.id);
                ...
                MessageConsumer source)
        {
            ...
            if (resolvedId != -1L)
            {
                logger.debug("resolvedId == -1L");
                newStream = new TestWelcomePathBindingFactory.TestSource(source, routeId, initialId, replyId, resolvedId)::onMessage;
            }
...
private final class TestSource
{
    ...
    private final TestWelcomePathBindingFactory.TestSource.TestTarget target;
    ...
    private TestSource(
            ...
            long resolvedId)
    {
        logger.debug("Instance of TestSource Created");
        this.source = source;
        this.routeId = routeId;
        this.initialId = initialId;
        this.replyId = replyId;
        this.target = new TestWelcomePathBindingFactory.TestSource.TestTarget(resolvedId);
    }
    ...
    private void onMessage(
            ...
            int length)
    {
        logger.debug("msgTypeId = " + msgTypeId);
        switch (msgTypeId)
        {
    ...
    private final class TestTarget
    {
        ...
        private final TestWelcomePathBindingFactory.TestSource source;
                ...
                long routeId)
        {
            ...
            this.source = TestWelcomePathBindingFactory.TestSource.this;
        }
            ...
            long traceId)
    {
        ...
        stream.accept(begin.typeId(), begin.buffer(), begin.offset(), begin.sizeof());
        logger.debug("doBegin = " + "routeId = " + routeId + "streamId = " + streamId);
    }

...
public class RouteWelcomePathAdapterTest {
    ...
    public void initJson()
    {
        JsonbConfig config = new JsonbConfig()
                .withAdapters(new RouteAdapter().adaptType("test_filesystem_server0"));
    ...
    public void shouldReadRoute()
    {
        String text =
                "{" +
                        "\"exit\": \"test_filesystem_server0\"" +
                        "}";

        RouteConfig route = jsonb.fromJson(text, RouteConfig.class);

        assertThat(route, not(nullValue()));
        assertThat(route.exit, equalTo("test_filesystem_server0"));
    }

    @Test
    public void shouldWriteRoute()
    {
        RouteConfig route = new RouteConfig("test_filesystem_server0");

        String text = jsonb.toJson(route);

        assertThat(text, not(nullValue()));
        assertThat(text, equalTo("{\"exit\":\"test_filesystem_server0\"}"));
    }
    ...
    public void shouldReadRouteGuarded()
    {
        ...
        assertThat(route.guarded.get(0).name, equalTo("test_filesystem_server0"));
